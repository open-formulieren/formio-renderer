...
<Canvas>
  ...
  <Story
    ...
    play={async ({canvasElement}) => {
      ...
      expect(await canvas.getByRole('alert')).toBeTruthy();
      // Too long.
      await userEvent.type(input, 'Lorem ipsum dolor sit amet, consectetur adipiscing elit.', {
        delay: 10,
      });
...
typically relies on the implementation). Please use the `onChange` handler to invoke a change to the
`submission` to re-trigger rendering and validation._
...
**Passing validators to render configuration**
