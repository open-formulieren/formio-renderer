...
const getValidationSchema: GetValidationSchema<NumberComponentSchema> = (
  ...
  const isLessOrEqualThanMaximum = (value: number | null): boolean => {
    if (max === undefined || value === null) return true;
    return value <= max;
  };

  const isGreaterOrEqualThanMinimum = (value: number | null): boolean => {
  ...
  let schema: z.ZodFirstPartySchemaTypes = z
    ...
    .refine(isLessOrEqualThanMaximum, {
      message: intl.formatMessage(NUMBER_GREATER_THAN_MAX_MESSAGE, {max}),
    })
    .refine(isGreaterOrEqualThanMinimum, {
