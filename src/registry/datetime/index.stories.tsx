...
export const MinMaxValidation: ValidationStory = {
  ...
  args: {
    ...
    componentDefinition: {
      ...
      label: 'Datetime',
      datePicker: {
        minDate: '2025-10-08T12:00:00',
        maxDate: '2025-10-10T19:00:00',
        showWeeks: false,
        startingDay: 0,
        initDate: '',
        minMode: 'day',
  ...
  play: async ({canvasElement, step, args}) => {
    ...
    await step('Date before date range', async () => {
      ...
      expect(
        await canvas.findByText(/Date must be greater than or equal to Wed Oct 08 2025 12:00:00/)
      ).toBeVisible();
    ...
    await step('Date after date range', async () => {
      ...
      await userEvent.click(button);
      // TODO-83: what is the correct validation message?
      expect(
        await canvas.queryByText(/Date must be smaller than or equal to Fri Oct 10 2025 19:00:00/)
      ).toBeVisible();
      // Still should have only been called once with the valid date from the previous step
      expect(args.onSubmit).toHaveBeenCalledWith({my: {date: '2025-10-08T15:00:00'}});
    });
  },
};
