import type {TimeInputSchema} from '@open-formulieren/types';

import type {IsEmpty} from '@/registry/types';

const isEmpty: IsEmpty<TimeInputSchema, string | string[]> = (_component, value) => {
  ...
  const isFalsy = (valueToCheck: unknown) =>
    ...
    valueToCheck === null ||
    valueToCheck.toString().trim().length === 0;
