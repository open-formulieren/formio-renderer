...
export const DatePickerTypeDateManually: Story = {
  ...
  play: async ({canvasElement, parameters}) => {
    ...
    expect(await canvas.queryByRole('dialog')).toBeNull();
    const textInput = canvas.getByLabelText('Date');
    await userEvent.type(textInput, '29/08/2025');

    // Ensure locale formatting is applied
    expect(await canvas.queryByRole('dialog')).toBeNull();
    expect(textInput).toHaveDisplayValue('29-8-2025');
...
export const DatePickerWithInitialValueAndValidationError: Story = {
  ...
  play: async ({canvasElement}) => {
    ...
    expect(canvas.getByLabelText('Date')).toHaveDisplayValue('29-08-2025');
