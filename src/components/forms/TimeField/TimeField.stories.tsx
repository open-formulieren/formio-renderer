...
export const InputSubmit: Story = {
  ...
  play: async ({canvasElement, parameters}) => {
    ...
    await userEvent.type(canvas.getByLabelText('Second'), '30');
...
export const InputWithInitialValue: Story = {
  ...
  play: async ({canvasElement}) => {
    ...
    expect(canvas.getByLabelText('Second')).toHaveDisplayValue('00');
...
export const InputReflectsExternalUpdates: Story = {
  ...
  play: async ({canvasElement, step}) => {
    ...
    await step('Assert initial state', () => {
      ...
      expect(canvas.getByLabelText('Second')).toHaveDisplayValue('');
    });
    ...
    await step('Change form field value from external trigger', async () => {
      ...
      expect(canvas.getByLabelText('Minute')).toHaveDisplayValue('03');
      expect(canvas.getByLabelText('Second')).toHaveDisplayValue('59');
    });
...
export const LeavesInvalidInputAlone: Story = {
  ...
  play: async ({canvasElement, parameters}) => {
    ...
    await userEvent.type(canvas.getByLabelText('Minute'), '13');
    await userEvent.type(canvas.getByLabelText('Second'), '55555');
...
export const ValidateOnBlur: Story = {
  ...
  play: async ({canvasElement}) => {
    ...
    const secondInput = canvas.getByLabelText('Second');
    await userEvent.type(secondInput, '55555');
    expect(canvas.queryByText('Always invalid')).not.toBeInTheDocument();
    expect(container).not.toHaveAttribute('aria-invalid');
...
export const InputInvalidMinute: Story = {
  ...
  play: async ({canvasElement, parameters}) => {
    ...
    await userEvent.type(canvas.getByLabelText('Second'), '59');
...
export const InputInvalidSecond: Story = {
  args: {
    name: 'test',
    label: 'Test time field',
    isDisabled: false,
  ...
  play: async ({canvasElement, parameters}) => {
    ...
    await userEvent.type(canvas.getByLabelText('Hour'), '23');
    await userEvent.type(canvas.getByLabelText('Minute'), '59');
    await userEvent.type(canvas.getByLabelText('Second'), '60');
...
export const WithoutLeadingZero: Story = {
  ...
  play: async ({canvasElement, parameters}) => {
    ...
    await userEvent.type(canvas.getByLabelText('Second'), '3');
